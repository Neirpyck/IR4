...
def json_from_list(list_restaurants, list_places, list_services, list_images, list_url):
    ...
    return [
        {
            "name": list_restaurants[i],
            "city": list_places[i],
            "services": [service for service in list_services[i]],
            "image": list_images[i],
            "url": list_url[i],
        }
...
def get_detail(restaurant):
    ...
    for restaurant in data:
        ...
        page = bs4.BeautifulSoup(request_text, "lxml")
        data[restaurant]["phone"], data[restaurant]["description"], = (
            get_phone(page),
...
def main():
    ...
    for i in range(1, 11):
        (
            ...
            list_images,
            list_url,
        ) = get_all_from_page_n(i)
        data = dict(
            data,
            **json_from_list(
                list_restaurants, list_places, list_services, list_images, list_url
            )
        )
        # TODO: Remove
        print("done bases for resto" + str(i))
    ...
    for restaurant in data_out:
        ...
        print("done details for resto" + str(i))
...
def main2():
    ...
    for i in range(1, 11):
        ...
        data.append(json_from_list(
                list_restaurants, list_places, list_services, list_images, list_url
            ))
        break

    data_dict = {'restaurants': data[0]}
